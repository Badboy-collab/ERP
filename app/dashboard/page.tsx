  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-100 flex flex-col font-sans">
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
        {/* Welcome Banner */}
        <div className="bg-gradient-to-r from-slate-900 via-emerald-950/60 to-slate-900 border border-slate-200 rounded-3xl p-6 sm:p-8 relative overflow-hidden shadow-2xl">
          <div className="relative z-10 max-w-3xl space-y-3">
            ...
            <div className="pt-2 flex flex-wrap gap-4">
              ...
              <Link
                href="/reports/stock"
                className="bg-slate-100 hover:bg-slate-200 text-slate-200 font-semibold px-5 py-2.5 rounded-xl border border-slate-300 transition-all text-sm"
              >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-xl flex items-center justify-between">
            <div>
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-xl flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-black text-blue-400 mt-1">
                {activeLotsCount}{" "}
                <span className="text-xs text-slate-500 font-normal">lots</span>
              </p>
            </div>
            ...
            <table className="w-full text-left text-sm">
              ...
              <tbody className="divide-y divide-slate-800/60 font-medium">
                {expiryReport.slice(0, 5).map((lot) => (
                  <tr key={lot.lot_id} className="hover:bg-slate-100/40 transition-colors">
                    <td className="p-3.5 font-mono text-emerald-600 font-bold">{lot.lot_no}</td>
                    <td className="p-3.5 text-xs text-slate-600 font-semibold">{lot.depot_name}</td>
                    <td className="p-3.5">
                    ...
                    <td className="p-3.5 text-right">
                      {lot.status === "URGENT" && (
                        <span className="px-2.5 py-1 text-xs font-bold rounded-full bg-rose-50 text-rose-600 border border-rose-800 animate-pulse">
                          🚨 URGENT (&le;10d)
                        </span>
                      )}
                      {lot.status === "WARNING" && (
                        <span className="px-2.5 py-1 text-xs font-bold rounded-full bg-amber-50 text-amber-600 border border-amber-800">
                          ⚠️ WARNING (&le;20d)
                        </span>
                      )}
                      {lot.status === "CAUTION" && (
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-xl space-y-4">
          <div className="flex items-center justify-between border-b border-slate-200 pb-4">
            ...
            <Link href="/reports/stock" className="text-xs text-emerald-600 hover:underline font-semibold">
            ...
            <table className="w-full text-left text-sm">
              <thead className="bg-slate-50 text-slate-500 text-xs font-semibold uppercase tracking-wider">
                <tr>
                  ...
                  <th className="p-3.5 text-right text-rose-600">Sales (Kg)</th>
                  <th className="p-3.5 text-right text-amber-600">Return (Kg)</th>
                  <th className="p-3.5 text-right font-bold text-emerald-600">Balance (Kg)</th>
                ...
                {realTimeStock.slice(0, 10).map((row) => (
                  <tr key={row.product_id} className="hover:bg-slate-100/40 transition-colors">
                    <td className="p-3.5 font-semibold text-slate-900">
                      <span className="text-[10px] text-emerald-600 block font-normal uppercase">
                        {row.category}
                      </span>
                      {row.name}
                    </td>
                    <td className="p-3.5 font-mono text-slate-500">{row.code}</td>
                    <td className="p-3.5 text-right font-mono text-slate-500">{row.opening_kg.toLocaleString()}</td>
                    ...
                    <td className="p-3.5 text-right font-mono text-rose-600">-{row.sales_kg.toLocaleString()}</td>
                    <td className="p-3.5 text-right font-mono text-amber-600">-{row.return_kg.toLocaleString()}</td>

        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <div>
            {totalDiscrepancies === 0 ? (
              <span className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-emerald-50 text-emerald-300 border border-emerald-200 font-bold text-sm">
                <CheckCircle2 className="w-5 h-5 text-emerald-600" />
                All Stock Batches Perfectly Balanced
              </span>
            ) : (
              <span className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-rose-50 text-rose-300 border border-rose-800 font-bold text-sm animate-bounce">
            ...
            <table className="w-full text-left text-sm">
              ...
              <tbody className="divide-y divide-slate-800/60 font-medium">
                {reconciliation.map((item) => (
                  <tr key={item.product_id} className="hover:bg-slate-100/40 transition-colors">
                    <td className="p-4 font-mono text-emerald-600 font-bold">{item.code}</td>
                    ...
                    <td className="p-4 text-right font-mono font-extrabold text-base">
                      {item.discrepancy === 0 ? (
                        ...
                        <span className="text-amber-600">+{item.discrepancy}</span>
                      ) : (
                        <span className="text-rose-600">{item.discrepancy}</span>
                      )}
                    </td>
                    <td className="p-4 text-center">
                      {item.isBalanced ? (
                        ...
                        <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold bg-rose-50 text-rose-600 border border-rose-800">

...
import {
  ...
  UserCircle, ChevronDown, X, ShieldCheck, Users, DollarSign, ArrowLeft
} from "lucide-react";
      ...
      <header className="bg-white text-slate-900 border-b border-slate-200 sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            ...
            {!isMinimal && (
              <nav className="hidden md:flex items-center space-x-1 flex-1 overflow-x-auto mx-4 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
                <Link href="/dashboard" className="px-3 py-2 rounded-lg text-sm font-medium bg-slate-100 text-slate-800 hover:bg-slate-200 transition-colors flex items-center space-x-1.5 shadow-sm flex-shrink-0">
                  <Layers className="w-4 h-4 text-emerald-600" />
                  <span>Hub</span>
                </Link>
                {hasPermission(currentUser, 'can_create_sales') && (
                  <Link href="/pos" className="px-3 py-2 rounded-lg text-sm font-medium bg-emerald-600 hover:bg-emerald-500 text-slate-900 shadow-md shadow-emerald-900/50 transition-all flex items-center space-x-1.5 flex-shrink-0">
                ...
                {hasPermission(currentUser, 'can_create_do') && (
                  <div className="relative" onMouseLeave={() => setOpenDropdown(null)}>
                    <button 
                      ...
                      className="px-3 py-2 rounded-lg text-sm font-medium text-slate-600 hover:text-slate-900 hover:bg-slate-100 transition-colors flex items-center space-x-1.5 flex-shrink-0"
                    >
                    ...
                    {openDropdown === 'operations' && (
                      <div className="absolute left-0 top-full mt-1 w-48 bg-white border border-slate-200 rounded-lg shadow-xl z-50 flex flex-col p-1">
                ...
                {hasPermission(currentUser, 'can_view_accounting') && (
                  <Link href="/accounting" className="px-3 py-2 rounded-lg text-sm font-medium text-slate-600 hover:text-slate-900 hover:bg-slate-100 transition-colors flex items-center space-x-1.5 flex-shrink-0">
                ...
                {currentUser?.role === "SUPER_ADMIN" && (
                  <Link href="/admin" className="px-3 py-2 rounded-lg text-sm font-medium text-amber-600 hover:bg-amber-50/40 border border-amber-800/60 transition-colors flex items-center space-x-1.5 flex-shrink-0">
            ...
            <div className="flex items-center space-x-3 relative flex-shrink-0">
              {currentUser?.role === "SUPER_ADMIN" && (
                <button
                  ...
                  className="hidden sm:flex items-center gap-1.5 text-xs bg-slate-800 text-white hover:bg-slate-700 px-3 py-2 rounded-lg font-bold transition-colors shadow-sm flex-shrink-0"
                >
